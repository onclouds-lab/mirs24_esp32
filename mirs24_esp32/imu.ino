void imu_setup(){
 if(!bno.begin())
  ...
  vlt_msg.data.size = 9; // メッセージ配列のサイズを2に設定
  vlt_msg.data.data = (double *)malloc(enc_msg.data.size * sizeof(double)); // 配列のメモリを確保
  vlt_msg.data.data[0] = 0;
  vlt_msg.data.data[1] = 0;
  vlt_msg.data.data[2] = 0;
  vlt_msg.data.data[3] = 0;
  vlt_msg.data.data[4] = 0;
  vlt_msg.data.data[5] = 0;
  vlt_msg.data.data[6] = 0;
  vlt_msg.data.data[7] = 0;
  vlt_msg.data.data[8] = 0;
  delay(1000);
}
